...
440 S=INT(RND*16)+15
450 RESTORE 6090
460 FOR X=1 TO (INT(RND*4)+1)*2-1
470 READ D8,D9
480 NEXT X
490 FOR X=1 TO S
500 X1=INT(RND*20)+1
510 X2=INT(RND*20)+1
520 IF a1(X1,X2)<>0 THEN 500
530 a1(X1,X2)=3
540 NEXT X
550 PRINT "YOU MUST DESTROY";S;" ENEMY SHIPS"
    print "TO WIN."
560 REM *** HEADQUARTERS ***
570 S3=INT(RND*20)+1
580 S4=INT(RND*20)+1
590 IF a1(S3,S4)<>0 THEN 570
600 a1(S3,S4)=4
610 REM *** UNDERWATER MINES ***
620 FOR X=1 TO INT(RND*8)+8
630 X1=INT(RND*20)+1
640 X2=INT(RND*20)+1
650 IF a1(X1,X2)<>0 THEN 630
660 a1(X1,X2)=5
670 NEXT X
...
700 X1=INT(RND*18)+2
710 X2=INT(RND*18)+2
720 IF a1(X1,X2)<>0 THEN 700
730 a1(X1,X2)=6
740 RESTORE 6090
750 FOR Y=1 TO INT(RND*8)+1
760 READ M1,M2
770 NEXT Y
780 NEXT X
...
1100 GOTO 890
1110 D1=1-((.23+RND/10)*(-(D0<=50)))
1120 GOSUB 6090
1130 PRINT "POWER AVAILABLE=";P;". POWER TO USE";
1140 INPUT P1
1150 IF P1<0 OR P1>P THEN 1130
1160 IF P1 <= 1000 THEN 1210
1170 IF RND<.43 THEN 1210
1180 PRINT "ATOMIC PILE GOES SUPERCRITICAL!!!"
1190 PRINT "HEADQUARTERS WILL WARN ALL SUBS TO"
     print "STAY AWAY FROM RADIOACTIVE AREA!!!"
1200 GOTO 6180
1210 X=S1
1220 Y=S2
1230 Q1=1
...
1270 GOTO 1340
1280 ON a1(X+X1,Y+Y1)+1 GOTO 1290,1330,1630,1390,1440,1470,1490
1290 X=X+X1
1300 Y=Y+Y1
1310 P=P-100
1320 GOTO 1520
...
1560 IF a1(X,Y)<>6 THEN 1610
1570 IF RND<.25 THEN 1500
1580 IF Q1=0 THEN 1610
1590 PRINT "YOU JUST HAD A NARROW ESCAPE WITH A"
     print "SEA MONSTER!!"
1600 Q1=0
1610 NEXT Y3
1620 NEXT X3
1630 NEXT X2
1640 PRINT "NAVIGATION COMPLETE."
     print "POWER LEFT=";P;"."
1650 IF P>0 THEN 1340
1660 PRINT "ATOMIC PILE HAS GONE DEAD!!! SUB SINKS,"
     print "CREW SUFFOCATES"
1670 GOTO 6180
1680 REM *** #1: SONAR ***
...
1820 FOR Y=1 TO 20
1830 DATA " ","*","X","S","H","$","#"
1840 IF a1(X,Y)<>0 THEN 1880
1850 IF X<>1 AND X<>20 AND Y<>1 AND Y<>20 THEN 1880
1860 PRINT ".";
1870 GOTO 1950
1880 RESTORE 1830
1890 FOR X1=1 TO a1(X,Y)+1
1900 READ AQ$
1910 NEXT X1
1920 IF D0<50 AND RND<.23 AND a1(X,Y)<>1 AND a1(A,Y)<>2 THEN 1860
1930 IF RND<.15 AND a1(X,Y)>2 THEN 1860
1940 PRINT AQ$;
1950 NEXT Y
1960 PRINT
1970 NEXT X
...
2100 IF a1(S1+X1*X4,S2+Y1*X4)<>3 THEN 2130
2110 X3=X3+1
2120 B(X3)=X4
2130 NEXT X4
...
2310 GOTO 880
2320 IF D0<2000 THEN 2360
2330 IF RND>.5 THEN 2360
2340 PRINT "PRESSURE IMPLODES UPON FIRING..."
     print "YOU'RE CRUSHED!!"
2350 GOTO 6180
2360 GOSUB 6080
2370 X=S1
2380 Y=S2
2390 FOR X2=1 TO INT(7+5*(-(D0>50))-RND*4+.5)
2400 IF X+X1>0 AND X+X1<21 AND Y+Y1>0 AND Y+Y1<21 THEN 2460
2410 PRINT "TORPEDO OUT OF SONAR RANGE..."
     print "INEFFECTUAL."
2420 T=T-1
2430 P=P-150
2440 IF P>0 THEN 4690
2450 GOTO 1660
2460 ON a1(X+X1,Y+Y1)+1 GOTO 2470,2510,2650,2540,2580,2610,2630
2470 X=X+X1
2480 Y=Y+Y1
2490 PRINT "..!..";
2500 GOTO 2650
...
2730 PRINT "NOT ENOUGH CREW TO LAUNCH A MISSILE."
2740 GOTO 880
2750 IF M<>0 THEN 2780
2760 PRINT "NO MISSILES LEFT."
2770 GOTO 880
2780 IF D0>50 AND D0<2000 THEN 2850
2790 PRINT "RECOMMEND THAT YOU DO NOT FIRE AT THIS"
     print "DEPTH...PROCEED";
2800 INPUT AQ$: AQ$=ucase$(AQ$)
2810 IF LEFT$(AQ$,1)="N" THEN 880
2820 IF RND<.5 THEN 2850
2830 PRINT "MISSILE EXPLODES UPON FIRING!!"
     print "YOU'RE DEAD!!"
2840 GOTO 6180
2850 GOSUB 6080
2860 PRINT "FUEL (LBS.)";
...
3040 D5=D5-(a1(X,Y)=5)
3050 D6=D6-(a1(X,Y)=1)
3060 IF a1(X,Y)<>4 THEN 3100
3070 PRINT "YOU'VE JUST DESTROYED YOUR HEADQUARTERS!"
3080 D3=0: S4=0: D2=0
3090 GOTO 3130
3100 IF a1(X,Y)<>2 THEN 3130
...
3190 PRINT "YOU DESTROYED";D5;"MINES."
3200 IF D4=0 THEN 3220
3210 PRINT "YOU GOT";D4;" SEA MONSTERS!"
3220 PRINT "YOU DESTROYED";D3;" ENEMY SHIPS!!!"
3230 S=S-D3
3240 GOTO 2940
...
3300 PRINT "THERE ARE NOT ENOUGH CREW TO WORK"
     print "THE CONTROLS."
3310 GOTO 880
...
3360 GOTO 6180
3370 P=P-INT(ABS((D0-D1)/2+.5))
3380 PRINT "MANUEVER COMPLETE. POWER LOSS=";INT(ABS((D0-D1)/2+.5))
3390 D0=D1
3400 GOTO 4690
...
3560 INPUT AQ$: AQ$=ucase$(AQ$)
3570 IF LEFT$(AQ$,1)="N" THEN 3670
3580 PRINT "           (+ GOOD, 0 NEUTRAL, - BAD)"
     PRINT "   ITEM            DAMAGE"
3590 PRINT "   ----            ------"
...
3760 GOTO 880
3770 IF SQR((S1-S3)^2+(S2-S4)^2) <= 2 AND D0<51 THEN 3800
3780 PRINT "UNABLE TO COMPLY WITH DOCKING ORDERS."
3790 GOTO 880
3800 PRINT "DIVERS FROM HEAD QUARTERS BRING OUT"
     print "SUPPLYS AND MEN."
3810 P=4000
3820 T=8
3830 M=2
3840 F=1500
...
3900 PRINT "HATCHES INACCESSIBLE."
     print "NO SABOTAGES POSSIBLE."
3910 GOTO 880
...
4050 GOTO 880
4060 PRINT "THERE ARE";D3;" SHIPS IN RANGE."
4070 PRINT "HOW MANY MEN ARE GOING";
4080 INPUT Q1
...
4160 IF D3/Q1>1-RND AND RND+D3/Q1<.9 THEN 4220
4170 IF a1(X,Y)<>3 THEN 4220
4180 D6=D6+1
4190 a1(X,Y)=0
4200 S=S-1
...
4220 NEXT Y
4230 NEXT X
4240 PRINT D6;" SHIPS WERE DESTROYED."
4250 D6=0: D7=0
4260 FOR X=1 TO Q1
4270 D7=D7-(RND>.6)
4280 NEXT X
...
4330 PRINT "A SEA MONSTER SMELLS THE MEN ON THE"
4340 PRINT  WAY BACK! ";D7;" MEN WERE EATEN!"
4350 C=C-D7
4360 PRINT D6;"MEN WERE LOST THROUGH ACCIDENTS."
4370 C=C-D6
4380 P=P-INT(10*Q1+RND*10)
4390 GOTO 4700
...
4460 GOTO 880
4470 PRINT "OPTION? (1=FUEL TO POWER,"
     print "         2=POWER TO FUEL)";
4480 INPUT O
...
4620 P=P-C1
4630 F=F+INT(C1*3)
4640 PRINT "CONVERSION COMPLETE. POWER=";P
     print "                     FUEL=";F
4650 GOTO 4690
...
4740 IF a1(X,Y)<>3 THEN 4760
4750 Q=Q+(RND/SQR((S1-X)^2+(S2-Y)^2))
4760 NEXT Y
4770 NEXT X
4780 IF Q THEN 4810
...
5030 PRINT "HEAVY DAMAGE!! REPAIRS IMMEDIATE!!!"
5040 P=P-(200+INT(RND*76))
5050 FOR X=1 TO 4+INT(RND*2)
5060 Y=INT(RND*9)+1
5070 D(Y)=D(Y)-RND*11
5080 NEXT X
5090 GOTO 5210
5100 PRINT "DAMAGE CRITICAL!!!!   WE NEED HELP!!!"
5110 AQ$="VRAVUKXCNVPCRHFDRSAXQURLQTRHXYACVFZYITLCBSSYYKDQIPCAEGQGPCNOTSIO"
5120 X=INT(RND*16)+1
5130 PRINT "SEND 'HELP' IN CODE. HERE IS THE CODE:"
     print MID$(AQ$,X,4);
5134 FOR I=1 TO 500:NEXT I
5136 PRINT CHR$(13);TAB(19);"XXXX";CHR$(13);TAB(19);"****"
5140 INPUT "ENTER CODE";B$
5150 PRINT
5160 IF B$<>MID$(AQ$,X,4) THEN 5190
5170 PRINT "FAST WORK!! HELP ARRIVES IN TIME TO"
     print "SAVE YOU!!!"
5180 GOTO 5040
...
5890 GOTO 6260
5900 PRINT "A SEA MONSTER ATE YOU HEADQUARTERS!!"
5910 S3=0: S4=0: D2=0
5920 GOTO 5730
5930 IF RND<.75 THEN 5760
5940 PRINT "*** A SEA MONSTER FIGHT!!! ";
5950 IF RND<.8 THEN 5980
...
6030 FOR Y=1 TO 9
6040 X=INT(RND*9)+1
6050 D(X)=D(X)+(RND*(2+RND*2))*(1+(-(D0<51) OR -(D0>2000)))*(-(D(X)<3))
6060 NEXT Y
6070 GOTO 880
...
6170 RETURN
6180 REM *** DESTROYED ? ***
6190 PRINT "THERE ARE STILL";S;" ENEMY SHIPS LEFT."
6200 PRINT "YOU WILL BE DEMOTED TO THE RANK OF DECK
     print "SCRUBBER!!!"
6210 PRINT "WANT ANOTHER GAME";
6220 INPUT AQ$:AQ$=ucase$(AQ$)
6230 IF LEFT$(AQ$,1)<>"Y" THEN END
6240 GOTO 310
6250 END
6260 PRINT "GOOD WORK!!!  YOU GOT THEM ALL!!!"
6270 PRINT "PROMOTION AND COMMENDATIONS WILL BE
     print "GIVEN IMMEDIATELY!!!"
6280 GOTO 6210
